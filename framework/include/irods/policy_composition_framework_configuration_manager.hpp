#ifndef IRODS_POLICY_COMPOSITION_FRAMEWORK_CONFIGURATION_MANAGER_HPP
#define IRODS_POLICY_COMPOSITION_FRAMEWORK_CONFIGURATION_MANAGER_HPP
...
namespace irods::policy_composition::policy_engine
{
	...
	class configuration_manager
	{
	  ...
	  public:
		configuration_manager(const std::string _in, const json& _cj)
			: plugin_cfg_{_in}
			, call_cfg_{_cj}
		{
		} // ctor
		...
		template <typename T>
		auto get(const std::string& _key, const T& _default) const
		{
		...
		auto get(const std::string& _key, const char _default[]) -> std::string const
		{
			const auto& tmp_cfg = plugin_cfg_.plugin_configuration;
...
#endif // IRODS_POLICY_COMPOSITION_FRAMEWORK_CONFIGURATION_MANAGER_HPP
